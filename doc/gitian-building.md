...
*Setup instructions for a Gitian build of Outastracoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Outastracoin
...
- [Building Outastracoin Core](#building-outastracoin-core)
...
Clone the git repositories for outastracoin and Gitian.
...
in the outastracoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Outastracoin Core
----------------

To build Outastracoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the outastracoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/outastracoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/outastracoin-project/outastracoin
...
For example:
```bash
URL=https://github.com/thrasher-/outastracoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit outastracoin=${COMMIT} --url outastracoin=${URL} ../outastracoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit outastracoin=${COMMIT} --url outastracoin=${URL} ../outastracoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit outastracoin=${COMMIT} --url outastracoin=${URL} ../outastracoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the outastracoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../outastracoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/outastracoin
SIGPATH=/some/root/path/outastracoin-detached-sigs

./bin/gbuild --url outastracoin=${BTCPATH},signature=${SIGPATH} ../outastracoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.oac` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/outastracoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/outastracoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/outastracoin-osx-build.assert
```
...
[gitian.sigs.oac](https://github.com/outastracoin-project/gitian.sigs.oac/) repository, or if that's not possible create a pull
